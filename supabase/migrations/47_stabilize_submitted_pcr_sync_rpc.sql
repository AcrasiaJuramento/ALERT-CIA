-- Stabilize PCR upload after removing the app-level "Completed" PCR status.
-- Completed/Submitted Locally payloads are normalized to submitted, and optional
-- age/date/time fields are parsed defensively so one bad optional value does not
...
begin
  ...
  target_status := case
    when raw_status in ('verified') then 'verified'::public.pcr_status
    when raw_status in ('rejected') then 'rejected'::public.pcr_status
  ...
  values (
    ...
    case when target_status = 'submitted' then coalesce(nullif(report_payload->>'submittedAt', '')::timestamptz, now()) else null end,
    null,
    auth.uid(),
    auth.uid()
  )
  ...
  if target_status = 'submitted' then
    ...
    set patient_name = coalesce(nullif(report_payload->>'patientName', ''), patient_name),
        ...
        updated_at = now()
    where (
        nullif(report_payload->>'dispatchPatientId', '') is not null
        and id = nullif(report_payload->>'dispatchPatientId', '')::uuid
      )
       or (
