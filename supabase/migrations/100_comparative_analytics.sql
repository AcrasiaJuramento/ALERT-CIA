-- Add hospital refusal/not-admitting records to the active aggregated Operations analytics.
...
begin
  ...
  with p_base as materialized (
    select p.id, p.response_id, p.status::text, p.triage, p.incident_nature,
      p.emergency_types, p.trauma_types, p.hospital_name, p.transfer_reason, r.response_number,
      ...
      nullif(public.alert_cia_safe_jsonb(p.notes)#>>'{__alertCiaExtended,refusalFacility}', '') refusal_facility,
      coalesce(
        nullif(public.alert_cia_safe_jsonb(p.notes)#>>'{__alertCiaExtended,waiverReason}', ''),
        nullif(p.transfer_reason, ''),
        'No reason recorded'
      ) refusal_reason,
    ...
    and (location_scope='all' or (location_scope='missing' and coalesce(barangay,'')='')
      or (location_scope='echague' and municipality ilike '%echague%')
      or (location_scope='outside' and coalesce(barangay,'')<>'' and coalesce(municipality,'') not ilike '%echague%'))
  ), comparison_i as materialized (
    select * from i_base where (location_scope='all' or (location_scope='missing' and coalesce(barangay,'')='')
      or (location_scope='echague' and municipality ilike '%echague%')
      or (location_scope='outside' and coalesce(barangay,'')<>'' and coalesce(municipality,'') not ilike '%echague%'))
  ), comparison_p as materialized (
    select * from p_base where (location_scope='all' or (location_scope='missing' and coalesce(barangay,'')='')
      or (location_scope='echague' and municipality ilike '%echague%')
      or (location_scope='outside' and coalesce(barangay,'')<>'' and coalesce(municipality,'') not ilike '%echague%'))
  ), comparison_d as materialized (
    select * from d_base where (location_scope='all' or (location_scope='missing' and coalesce(barangay,'')='')
      or (location_scope='echague' and municipality ilike '%echague%')
      or (location_scope='outside' and coalesce(barangay,'')<>'' and coalesce(municipality,'') not ilike '%echague%'))
  ), mvc as materialized (
  ...
  select jsonb_build_object(
    ...
    'hospitalRefusalRecords',coalesce((select jsonb_agg(jsonb_build_object(
      'id',id,
      'responseNumber',response_number,
      'hospitalName',coalesce(nullif(trim(refusal_facility),''),'No Facility Recorded'),
      'transferReason',refusal_reason,
      'date',date,
      'status',status
    ) order by date desc nulls last) from p where refusal='Refusal documented'),'[]'::jsonb),
    ...
    'comparative',jsonb_build_object(
      'currentMonth',jsonb_build_object(
        'label',to_char(current_date,'Mon YYYY'),
        'incidents',(select count(*) from comparison_i where date>=date_trunc('month',current_date)::date and date<(date_trunc('month',current_date)+interval '1 month')::date),
        'dispatches',(select count(*) from comparison_d where date>=date_trunc('month',current_date)::date and date<(date_trunc('month',current_date)+interval '1 month')::date),
        'pcr',(select count(*) from comparison_p where date>=date_trunc('month',current_date)::date and date<(date_trunc('month',current_date)+interval '1 month')::date),
        'total',(select count(*) from comparison_i where date>=date_trunc('month',current_date)::date and date<(date_trunc('month',current_date)+interval '1 month')::date)+(select count(*) from comparison_d where date>=date_trunc('month',current_date)::date and date<(date_trunc('month',current_date)+interval '1 month')::date)+(select count(*) from comparison_p where date>=date_trunc('month',current_date)::date and date<(date_trunc('month',current_date)+interval '1 month')::date)),
      'previousMonth',jsonb_build_object(
        'label',to_char(current_date-interval '1 month','Mon YYYY'),
        'incidents',(select count(*) from comparison_i where date>=(date_trunc('month',current_date)-interval '1 month')::date and date<date_trunc('month',current_date)::date),
        'dispatches',(select count(*) from comparison_d where date>=(date_trunc('month',current_date)-interval '1 month')::date and date<date_trunc('month',current_date)::date),
        'pcr',(select count(*) from comparison_p where date>=(date_trunc('month',current_date)-interval '1 month')::date and date<date_trunc('month',current_date)::date),
        'total',(select count(*) from comparison_i where date>=(date_trunc('month',current_date)-interval '1 month')::date and date<date_trunc('month',current_date)::date)+(select count(*) from comparison_d where date>=(date_trunc('month',current_date)-interval '1 month')::date and date<date_trunc('month',current_date)::date)+(select count(*) from comparison_p where date>=(date_trunc('month',current_date)-interval '1 month')::date and date<date_trunc('month',current_date)::date)),
      'currentYear',jsonb_build_object(
        'label',to_char(current_date,'YYYY'),
        'incidents',(select count(*) from comparison_i where extract(year from date)=extract(year from current_date)),
        'dispatches',(select count(*) from comparison_d where extract(year from date)=extract(year from current_date)),
        'pcr',(select count(*) from comparison_p where extract(year from date)=extract(year from current_date)),
        'total',(select count(*) from comparison_i where extract(year from date)=extract(year from current_date))+(select count(*) from comparison_d where extract(year from date)=extract(year from current_date))+(select count(*) from comparison_p where extract(year from date)=extract(year from current_date))),
      'previousYear',jsonb_build_object(
        'label',to_char(current_date-interval '1 year','YYYY'),
        'incidents',(select count(*) from comparison_i where extract(year from date)=extract(year from current_date)-1),
        'dispatches',(select count(*) from comparison_d where extract(year from date)=extract(year from current_date)-1),
        'pcr',(select count(*) from comparison_p where extract(year from date)=extract(year from current_date)-1),
        'total',(select count(*) from comparison_i where extract(year from date)=extract(year from current_date)-1)+(select count(*) from comparison_d where extract(year from date)=extract(year from current_date)-1)+(select count(*) from comparison_p where extract(year from date)=extract(year from current_date)-1)),
      'monthly',(select jsonb_agg(jsonb_build_object(
        'month',to_char(make_date(2000,m,1),'Mon'),
        'currentIncidents',(select count(*) from comparison_i where extract(year from date)=extract(year from current_date) and extract(month from date)=m),
        'currentDispatches',(select count(*) from comparison_d where extract(year from date)=extract(year from current_date) and extract(month from date)=m),
        'currentPcr',(select count(*) from comparison_p where extract(year from date)=extract(year from current_date) and extract(month from date)=m),
        'currentTotal',(select count(*) from comparison_i where extract(year from date)=extract(year from current_date) and extract(month from date)=m)+(select count(*) from comparison_d where extract(year from date)=extract(year from current_date) and extract(month from date)=m)+(select count(*) from comparison_p where extract(year from date)=extract(year from current_date) and extract(month from date)=m),
        'previousIncidents',(select count(*) from comparison_i where extract(year from date)=extract(year from current_date)-1 and extract(month from date)=m),
        'previousDispatches',(select count(*) from comparison_d where extract(year from date)=extract(year from current_date)-1 and extract(month from date)=m),
        'previousPcr',(select count(*) from comparison_p where extract(year from date)=extract(year from current_date)-1 and extract(month from date)=m),
        'previousTotal',(select count(*) from comparison_i where extract(year from date)=extract(year from current_date)-1 and extract(month from date)=m)+(select count(*) from comparison_d where extract(year from date)=extract(year from current_date)-1 and extract(month from date)=m)+(select count(*) from comparison_p where extract(year from date)=extract(year from current_date)-1 and extract(month from date)=m)
