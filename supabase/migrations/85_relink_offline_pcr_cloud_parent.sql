-- Keep offline/LAN PCRs attached to the canonical cloud dispatch parent after
-- dispatcher and responder devices reconnect in different orders.

begin;

create index if not exists responses_client_generated_id_lookup_idx
  on public.responses(client_generated_id)
  where client_generated_id is not null and deleted_at is null;

create index if not exists dispatch_forms_client_generated_id_lookup_idx
  on public.dispatch_forms(client_generated_id)
  where client_generated_id is not null and deleted_at is null;

create index if not exists pcr_reports_client_generated_id_lookup_idx
...
declare
  ...
  target_response_client_id uuid;
  target_dispatch_form_id uuid;
  target_dispatch_response_id uuid;
  target_dispatch_client_id uuid;
  target_team_id uuid;
  ...
  raw_vital_time text;
  uuid_pattern constant text := '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{12}$';
begin
  ...
  if lower(coalesce(report_payload->>'responseClientId', '')) ~ uuid_pattern then
    target_response_client_id := (report_payload->>'responseClientId')::uuid;
  end if;
  ...
  if lower(coalesce(report_payload->>'dispatchClientId', '')) ~ uuid_pattern then
    target_dispatch_client_id := (report_payload->>'dispatchClientId')::uuid;
  end if;
  ...
  if target_response_id is null and target_response_client_id is not null then
    select r.id
    into target_response_id
    from public.responses r
    where (r.id = target_response_client_id or r.client_generated_id = target_response_client_id)
  ...
  select d.id, d.response_id
  into target_dispatch_form_id, target_dispatch_response_id
  from public.dispatch_forms d
  where d.deleted_at is null
    and (
      (target_dispatch_form_id is not null and d.id = target_dispatch_form_id and d.response_id = target_response_id)
      or (target_dispatch_client_id is not null and d.client_generated_id = target_dispatch_client_id)
      or d.response_id = target_response_id
    )
  order by
    case when d.response_id = target_response_id then 0 else 1 end,
    d.updated_at desc
  limit 1;

  target_response_id := coalesce(target_dispatch_response_id, target_response_id);
  ...
  on conflict (id) do update
    set response_id = excluded.response_id,
        dispatch_form_id = coalesce(excluded.dispatch_form_id, public.pcr_reports.dispatch_form_id),
        dispatch_patient_id = coalesce(excluded.dispatch_patient_id, public.pcr_reports.dispatch_patient_id),
