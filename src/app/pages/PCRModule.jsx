...
import { PCR_CORRECTION_TARGETS, correctionFindLabels, correctionStep } from "../utils/pcrCorrectionTargets";
...
const pcrAnchor = value => String(value || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
function Field({ label, children, wide = false, correctionKey }) { return <label data-pcr-field={correctionKey || pcrAnchor(label)} className={wide ? "md:col-span-2" : ""}><span className="block text-xs font-medium text-muted-foreground mb-1">{label}</span>{children}</label>; }
function Section({ title, children, correctionKey }) { return <section data-pcr-field={correctionKey || pcrAnchor(title)} className="relative overflow-visible rounded-xl border border-border"><h3 className="rounded-t-[inherit] px-4 py-2.5 bg-secondary text-sm font-bold text-foreground uppercase tracking-wide">{title}</h3><div className="p-4">{children}</div></section>; }
...
export default function PCRModule() {
  ...
  const activeCorrectionTargets = Array.isArray(form.correctionTargets) ? form.correctionTargets : [];
  const [correctionIndex, setCorrectionIndex] = useState(0);
  const activeCorrection = PCR_CORRECTION_TARGETS.find(item => item.key === activeCorrectionTargets[correctionIndex]);
  const openCorrection = index => {
    const safeIndex = Math.max(0, Math.min(activeCorrectionTargets.length - 1, index));
    const target = PCR_CORRECTION_TARGETS.find(item => item.key === activeCorrectionTargets[safeIndex]);
    setCorrectionIndex(safeIndex);
    if (target) setStep(target.step);
  };
  useEffect(() => {
    if (["Returned for Correction", "Returned to Field Officer"].includes(form.status) && form.correctionTargets?.length) {
      setCorrectionIndex(0);
      setStep(correctionStep(form.correctionTargets));
    }
  }, [form.id, form.status, form.correctionTargets]);
  useEffect(() => {
    if (!activeCorrection || !["Returned for Correction", "Returned to Field Officer"].includes(form.status)) return undefined;
    let highlighted;
    let note;
    const timer = window.setTimeout(() => {
      const anchors = [activeCorrection.key, ...correctionFindLabels(activeCorrection).map(pcrAnchor)];
      highlighted = anchors.map(anchor => document.querySelector(`[data-pcr-field="${anchor}"]`)).find(Boolean);
      if (!highlighted) return;
      highlighted.style.outline = '3px solid #dc2626';
      highlighted.style.outlineOffset = '3px';
      highlighted.style.backgroundColor = 'rgba(127, 29, 29, .22)';
      note = document.createElement('div');
      note.textContent = `Correction required: ${form.correctionReason || form.returnRemarks || 'Please update this field.'}`;
      Object.assign(note.style, { color: '#ffffff', background: '#991b1b', border: '1px solid #f87171', borderRadius: '8px', padding: '8px 10px', marginTop: '8px', fontSize: '12px', fontWeight: '700' });
      highlighted.appendChild(note);
      highlighted.scrollIntoView({ behavior: 'smooth', block: 'center' });
      highlighted.querySelector('input,textarea,select,button')?.focus?.({ preventScroll: true });
    }, 120);
    return () => { window.clearTimeout(timer); if (note?.parentNode) note.parentNode.removeChild(note); if (highlighted) { highlighted.style.outline = ''; highlighted.style.outlineOffset = ''; highlighted.style.backgroundColor = ''; } };
  }, [activeCorrection, form.correctionReason, form.returnRemarks, form.status, step]);
  useEffect(() => {
    let mounted = true;
  ...
  const pcrStatusKey = String(form.status || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '_');
  const pcrAlreadySubmitted = ['submitted', 'pending_dispatcher_review', 'accepted_by_dispatcher', 'linked_to_dispatch', 'pending_admin_verification', 'verified', 'completed'].includes(pcrStatusKey);
  ...
  const store = async status => {
    ...
    try {
      ...
      const errorMessage = error.code === '23505' || /already has a pending|duplicate|cannot be submitted|must be accepted before creating/i.test(error.message || '')
        ? "This record already has a pending request/submission."
        : error.message || "Unable to save PCR report.";
  ...
  return <div className="p-4 md:p-6 max-w-7xl mx-auto text-foreground">
    ...
    {["Returned for Correction", "Returned to Field Officer"].includes(form.status) && activeCorrectionTargets.length > 0 && <div className="mb-4 rounded-xl border-2 border-red-500 bg-slate-950 p-4 text-white shadow-lg"><div className="flex flex-wrap items-center justify-between gap-2"><div className="font-bold text-red-300">Admin corrections required</div><div className="rounded-full bg-red-600 px-2.5 py-1 text-xs font-bold text-white">{correctionIndex + 1} of {activeCorrectionTargets.length}</div></div><div className="mt-2 rounded-lg bg-white/10 px-3 py-2 text-sm font-semibold text-white">{form.correctionReason || form.returnRemarks}</div><div className="mt-3 flex flex-wrap gap-2">{activeCorrectionTargets.map((key,index) => { const target = PCR_CORRECTION_TARGETS.find(item => item.key === key); return target ? <button type="button" key={key} onClick={() => openCorrection(index)} className={`rounded-full border px-3 py-1.5 text-xs font-bold ${index === correctionIndex ? 'border-red-300 bg-red-600 text-white' : 'border-slate-500 bg-slate-800 text-slate-100'}`}>{target.label}</button> : null; })}</div>{activeCorrectionTargets.length > 1 && <div className="mt-3 flex justify-end gap-2"><button type="button" disabled={correctionIndex === 0} onClick={() => openCorrection(correctionIndex - 1)} className="rounded-lg bg-slate-700 px-3 py-2 text-xs font-bold text-white disabled:opacity-40">Previous correction</button><button type="button" disabled={correctionIndex === activeCorrectionTargets.length - 1} onClick={() => openCorrection(correctionIndex + 1)} className="rounded-lg bg-red-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-40">Next correction</button></div>}</div>}
