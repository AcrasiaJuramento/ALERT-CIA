...
export default function DispatchModule({ onBack }) {
  ...
  const dispatchStatusKey = String(form.status || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '_');
  const reversePcrStatusKey = String(linkedPCR?.status || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '_');
  const dispatchAlreadySent = !['', 'draft', 'dispatched'].includes(dispatchStatusKey)
    || (isReverseWorkflow && !['', 'draft', 'in_progress', 'accepted_by_dispatcher', 'returned_to_field_officer', 'returned_for_correction'].includes(reversePcrStatusKey));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(error.code === '23505' || /already has a pending|duplicate/i.test(error.message || '') ? "This record already has a pending request/submission." : error.message || "Unable to save dispatch form.");
    } finally {
      setDispatchAction("");
    }
  };
  ...
  const handleSendToFieldOfficer = async () => {
    ...
    try {
      ...
      toast.error(error.code === '23505' || /already has a pending|duplicate/i.test(error.message || '') ? "This record already has a pending request/submission." : error.message || "Unable to send dispatch.");
    } finally {
      setDispatchAction("");
    }
  };
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto text-foreground">
      ...
      <div className="sticky bottom-0 z-30 -mx-4 mt-5 border-t border-border bg-background/95 px-4 py-3 backdrop-blur md:-mx-6 md:px-6">
        <div className="mx-auto flex max-w-7xl flex-wrap justify-end gap-2">
          ...
          <button onClick={handleSave} disabled={Boolean(dispatchAction)} className="flex items-center gap-2 rounded-lg bg-secondary px-4 py-2 text-sm hover:bg-secondary/80 disabled:cursor-not-allowed disabled:opacity-60">
            <Save size={15} /> {dispatchAction === 'save' ? 'Saving...' : 'Save Draft'}
          </button>
          <button onClick={handlePrimarySend} disabled={Boolean(dispatchAction) || dispatchAlreadySent} className="flex items-center gap-2 rounded-lg bg-green-600 px-5 py-2 text-sm text-white hover:bg-green-500 disabled:cursor-not-allowed disabled:opacity-60">
            <Send size={15} /> {dispatchAction ? 'Processing...' : dispatchAlreadySent ? (isReverseWorkflow ? 'Sent to Admin' : 'Sent to Responding Team') : isReverseWorkflow ? 'Send to Admin' : 'Send to Responding Team'}
          </button>
        </div>
      </div>
